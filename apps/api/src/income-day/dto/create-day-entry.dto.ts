...
export class CreateDayEntryDto {
  ...
  @ApiProperty() @IsBoolean() walkIn!: boolean;
  @ApiProperty({ required: false }) @IsOptional() @IsString() tentId?: string;
  @ApiProperty({ required: false }) @IsOptional() @IsString() tentName?: string;

  @ApiProperty() @IsInt() @Min(0) paxBelow5!: number;
  @ApiProperty() @IsInt() @Min(0) pax5to10!: number;
  @ApiProperty() @IsInt() @Min(0) paxAbove10!: number;

  @ApiProperty({ required: false, description: 'Flat rupee discount off the computed gross amount' })
